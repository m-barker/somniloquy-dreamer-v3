...
def main(args):
    ...
    for t in range(n_rollouts):
        ...
        for index, state in enumerate(imagained_states):
            imagined_grid = agent._wm.heads["decoder"](state)[
                "flattened_occupancy_grid"
            ].mode()
            imagined_image = agent._wm.heads["decoder"](state)["image"].mode()
            ...
            # imagined_grid = np.clip(255 * imagined_grid, 0, 255).astype(np.uint8)
            ...
            cv2.imwrite(
                f"true_img_rollout_{t+1}_step_{index+1}.png",
