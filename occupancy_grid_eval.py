...
def main(args):
    ...
    for t in range(n_rollouts):
        ...
        for index, imagined_state, posterior_state in zip(
            ...
            imagined_image = agent._wm.heads["decoder"](imagined_state)["image"].mode()
            ...
            cv2.imwrite(
                os.path.join(output_path, f"true_img_rollout_{t+1}_step_{index+1}.png"),
            ...
            cv2.imwrite(
                os.path.join(
                    output_path, f"reconstructed_img_rollout_{t+1}_step_{index+1}.png"
                ),
        ...
        try:
            print(f"Reconstructed Narration: {narrator.narrate(reconstructed_grids)}")
        except Exception as e:
            print(e)
